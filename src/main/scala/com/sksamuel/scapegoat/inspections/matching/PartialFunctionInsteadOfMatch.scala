    ...
    override def postTyperTraverser = Some apply new context.Traverser {
      ...
      override def inspect(tree: Tree): Unit = {
        tree match {
          ...
          case Apply(_, List(Function(List(ValDef(_, name1, _, EmptyTree)), Match(name2, _)))) if name1.toString == name2.toString() => if (!isPFBind(name1)) warn(tree)
          case Apply(TypeApply(_, _), List(Function(List(ValDef(_, name1, _, EmptyTree)), Match(name2, _)))) if name1.toString == name2.toString() => if (!isPFBind(name1)) warn(tree)
          case TypeApply(_, List(Function(List(ValDef(_, name1, _, EmptyTree)), Match(name2, _)))) if name1.toString == name2.toString() => if (!isPFBind(name1)) warn(tree)
          // a => a match { case ...; case ... }
