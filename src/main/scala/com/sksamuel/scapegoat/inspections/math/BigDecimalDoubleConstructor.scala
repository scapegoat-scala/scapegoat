...
class BigDecimalDoubleConstructor extends Inspection {
  ...
  override def traverser(global: Global, feedback: Feedback): global.Traverser = new global.Traverser {
    ...
    private def warn(tree: Tree): Unit = {
      feedback.warn("Big decimal double constructor", tree.pos, Levels.Warning,
    ...
    override def traverse(tree: Tree): Unit = {
      tree match {
        case Apply(Select(pack, TermName("apply")), arg :: tail) if isBigDecimal(pack) && isFloatingPointType(arg) =>
          warn(tree)
        case Apply(Select(New(pack), nme.CONSTRUCTOR), arg :: tail) if isBigDecimal(pack) && isFloatingPointType(arg) =>
          warn(tree)
